...
template<class Key, class U, class Ref,class Ptr>
  struct Iterator_RB_Node
  {
    ...
    typedef Iterator_RB_Node<Key,U,Key&,Key*> iterator;
    typedef Iterator_RB_Node<Key,U,const Key&,const Key*> const_iterator;
    typedef Iterator_RB_Node<Key,U,Ref,Ptr> self;

    typedef Tree_Node<Key, U>* link_type;
	Key first;
	U   second;

    link_type node;
    /*下面这个节点是我自己加的，因为我设计的红黑树中节点分为普通节点和空节点
    **所有节点的指针域都不会是NULL，我们都用空节点来代替，其颜色都是黑色
     因此，在迭代器执行++，--操作时，为了符合终止条件，故在创建迭代器时候，
     要讲迭代器所指向的红黑树中表达的空节点指针传过来，不然，迭代器中的循环终止
     条件无法判断。
    ...
    */
    ...
    //construct
    Iterator_RB_Node(){}
    Iterator_RB_Node(link_type x,link_type y)
	{  
		node=x; 
		first = node->key;
		second = node->data;
    ...
    void increment()   //指针++操作
    {
      //当孩子有右节点时，++为右子树的最大左孩子节点
      ...
      else  //往父节点上寻找
      {
        ...
        while(parents!= nullNode && (node==parents->right))  //为父节点右孩子时，继续往上找
        {
          node=parents;
...
{  
    ...
    private:  
		typedef Tree_Node<KEY,U> RB_Node;
        public:  
            RB_Tree()  
            {  
                this->m_nullNode = (RB_Node*)malloc(sizeof(RB_Node));
				new(m_nullNode)RB_Node;
            ...
            */  
			...
			//中序非递归遍历
			RB_Node* TravalBegin1()
			{
				RB_Node* pNode = m_root;
				while(pNode->left != m_nullNode){
					pNode = pNode->left;
				}
				return pNode;
			}

			RB_Node* TravalNext1(RB_Node* node)
			{
				RB_Node* pNextNode = node;
				if (pNextNode == m_nullNode) return m_nullNode;

				if (node->right != m_nullNode){
						pNextNode = node->right;
						while(pNextNode->left != m_nullNode){
							pNextNode = pNextNode->left;
						}
						return pNextNode;
				}
				...
				if (parent->left == node){
					...
					while(parent != m_nullNode && parent->right == pNextNode){
						pNextNode = parent;
			...
			void Traval1()
			{
				RB_Node* pNode = TravalBegin1();
				while (pNode != m_nullNode){
					printf("key %d val %d \n", pNode->key, pNode->data);
					pNode = TravalNext1(pNode);
				}
			}

			//后序遍历
			RB_Node* TravalBegin2()
			{
				RB_Node* pNode = m_root;
				if (pNode == m_nullNode) return pNode;
			...
			RB_Node* TravalNext2(RB_Node* pNode)
			{
				RB_Node* parent = pNode->parent;
				//右子树返回根节点
				if (parent->right == pNode || parent->right == m_nullNode){
					return parent;
				}
				//左子树返回根节点的右子树最左节点
			...
			void Traval2()
			{
				RB_Node* pNode = TravalBegin2();
				while (pNode != m_nullNode){
					printf("key %d val %d \n", pNode->key, pNode->data);
					pNode = TravalNext2(pNode);
				}
			}
			...
			RB_Node* TravalNext(RB_Node* node)
			{
				RB_Node* parent = node->parent;
				...
				RB_Node* nextNode = node;

				while(true){
					//左子树
					if (parent == m_nullNode) return m_nullNode;
					if (parent->left == node){
						while(parent != m_nullNode && parent->left == nextNode && parent->right == m_nullNode){
							nextNode = nextNode->parent;
							parent = parent->parent;
						}

						if (parent == m_nullNode)
							return m_nullNode;

						if (parent->right == m_nullNode)
							parent = parent->parent;
						else
							return parent->right;
						...
						//右子树
						while(parent != m_nullNode && parent->right == nextNode){
							nextNode = nextNode->parent;
							parent = parent->parent;
						}
			...
			void Traval()
			{
				RB_Node* pNode = TravalBegin();
				while (pNode != m_nullNode){
					printf("key %d val %d \n", pNode->key, pNode->data);
					pNode = TravalNext(pNode);
				}
			}
			...
			{
				RB_Node* temp= m_root;
				while(temp->left != m_nullNode)
					temp=temp->left;
				return iterator(temp, m_nullNode);
			}
			iterator end(){  return iterator(m_nullNode, m_nullNode);}

			iterator minleft() {return begin();}
			iterator maxright()
			{
				RB_Node<T>* temp= m_root;
				while(temp->right!= m_nullNode)
					temp=temp->right;
				return iterator(temp,m_nullNode);
			}
    private:  
